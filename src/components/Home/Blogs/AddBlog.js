...
const CustomInput = ({
  ...
  return (
    <div className="relative mb-4">
      <label className="flex flex-row items-center justify-center gap-1 font-semibold">
        {label} {icon}
      </label>
      <input
...
export default function AddBlog() {
  ...
  const validateForm = () => {
    ...
    else if (title.split(" ").length > 25)
      newErrors.title = "Title should be a Maximum of 25 words";
    if (!description) newErrors.description = "Description is required";
    else if (description.split(" ").length > 3000)
      newErrors.description = "Description should be a maximum of 3000 words";
    else if (description.split(" ").length < 50)
      newErrors.description = "Description should be minimum of 50 words";
    if (!image) {
      newErrors.image = "Image is required!";
    } else if (!/^https?:\/\/\S+\.\S+$/.test(image)) {
      newErrors.image =
        "Image URL must start with http:// or https:// and must not contain spaces";
    }
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className={`w-full text-white font-semibold py-2 rounded-lg shadow-lg transition duration-300 ${
            isSubmitDisabled
